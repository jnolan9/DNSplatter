"""Load DN-Splatter model and render all outputs to disk"""
...
class RenderModel:
    ...
    split: Literal["all", "test", "eval"] = "all"
        ...
        if self.split in ["eval", "all"]:
            with torch.no_grad():
                ...
                for image_idx, data in enumerate(
                    ...
                    if "long_capture" in str(seq_name).split("/"):
                        image_name = "long_capture_" + image_name
                    if "short_capture" in str(seq_name).split("/"):
                        image_name = "short_capture_" + image_name
